...
const Home = () => {
  ...
  return (
    <div className={`max-w-6xl mx-auto ${isSmallDevice ? '-mt-[22rem]' : 'mt-[10%] md:-mt-10'} flex justify-between items-center h-screen`}>
      <div className="flex flex-col md:flex-row justify-center items-start w-full">
        ...
        <div className="relative w-full flex items-start flex-col md:flex-row">
          <div className="image-container flex-shrink-0 relative w-full md:w-[850px]">
            ...
            <h1 key={titleAnimationKey} className={`absolute -bottom-16 -left-[3.7rem] md:top-10 md:left-[75%] text-3xl md:text-9xl text-white font-bold font-mono z-10 animated-title ${isSmallDevice ? 'mt-4 text-center w-full' : ''}`} style={{ lineHeight: isSmallDevice ? '1' : '0.9', maxWidth: isSmallDevice ? '' : 'none' }}>{title}</h1>
          </div>
